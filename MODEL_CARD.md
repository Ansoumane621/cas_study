...
- Hugging Face model repo: `YOUR_USERNAME/YOUR_MODEL_NAME` (fill in after
  `push_to_hub.py`)
- Hugging Face Space: `YOUR_USERNAME/YOUR_SPACE_NAME` (fill in after
  deployment)
...
**TODO (fill in after the real training run on GPU):** final
train/validation loss curves, total training steps, wall-clock time,
hardware used.
...
| Metric | Value |
|--------|-------|
| Exact Match | TODO (fill in from evaluation.py output) |
| Token F1 | TODO |
| BLEU | TODO |
| ROUGE-L | TODO |
...
- Model Hub URL: TODO (after `push_to_hub.py`)
- Space URL: TODO (after deploying `hf_space/app.py`)
